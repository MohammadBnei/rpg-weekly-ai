...
<div>
	<div class="fixed w-full h-screen bg-cover" style="background-image: url(/background.png)">
		<div
			class="flex flex-col items-center justify-center w-full h-full min-h-screen backdrop-blur-md bg-gradient-to-br from-slate-900/80 to-black/90"
		/>
	</div>

	<div class="absolute inset-0 flex flex-col h-screen px-6 overflor-auto">
		<div in:fade|global class="w-full max-w-4xl mx-auto">
			<div class="w-full mb-8">
				...
				{#if recommendations.length > 0 && endStream}
					<button
						on:click={clearForm}
						class="flex items-center justify-center w-full h-10 p-3 mt-4 font-bold text-white rounded-full bg-white/20 hover:bg-white/30"
					>
						Clear Search
					</button>
				{/if}
			</div>
			<div class="w-full max-w-4xl mx-auto md:pb-20">
				{#if loading && !searchResponse && !recommendations}
					<div class="mt-8 mb-4 text-lg text-center fontsemibold">
						Please be patient as I think. Good things are coming 😎.
					</div>
				{/if}
				{#if error}
					<div class="mt-8 text-lg text-center text-red-500 fontsemibold">
						Woops! {error}
					</div>
				{/if}
				{#if recommendations}
